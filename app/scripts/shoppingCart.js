    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.id != null && item.sku != null && item.name != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.id, item.sku, item.name, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (id, sku, name, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(id, sku, name, price, quantity);
            this.items.push(item);
        }
...
// items in the cart
//
function cartItem(id, sku, name, price, quantity) {
    this.id = id;
    this.sku = sku;
    this.name = name;
    this.price = price * 1;
